...
describe("DeFiMathRates", function () {
  ...
  describe("compoundInterest", function () {
    ...
    describe("performance", function () {
      it("compoundInterest across 3×4×4 principals/rates/times — 425 gas", async function () {
        const { rates } = await loadFixture(deploy);
        ...
        assert.equal(avg, 425, `gas changed: ${avg} ≠ 425 — deterministic, update threshold if intentional`);
      });
    });
  });
  ...
  describe("presentValue", function () {
    ...
    describe("performance", function () {
      it("presentValue across 3×4×4 FVs/rates/times — 477 gas", async function () {
        const { rates } = await loadFixture(deploy);
        ...
        const avg = Math.round(totalGas / count);
        assert.equal(avg, 477, `gas changed: ${avg} ≠ 477 — deterministic, update threshold if intentional`);
      });
    });
  });
  ...
  describe("discreteToContinuous", function () {
    ...
    describe("performance", function () {
      it("discreteToContinuous across r in [-0.5, 0.5] — 574 gas", async function () {
        const { rates } = await loadFixture(deploy);
        let totalGas = 0, count = 0;
        for (let r = -0.5; r <= 0.5; r += 0.01) {
        ...
        const avg = Math.round(totalGas / count);
        assert.equal(avg, 574, `gas changed: ${avg} ≠ 574 — deterministic, update threshold if intentional`);
      });
    });
  });
  ...
  describe("internalRateOfReturn", function () {
    ...
    describe("performance", function () {
      it("internalRateOfReturn on 4-cashflow annuity + 12-cashflow bond — 16155 / 46617 gas", async function () {
        const { rates } = await loadFixture(deploy);
        ...
        // 4-cashflow annuity: invest 1000, receive 300 at each of years 1, 2, 3, 4
        {
          ...
          assert.equal(gas, 16155, `gas changed: ${gas} ≠ 16155 (4-cashflow) — deterministic, update threshold if intentional`);
        }
        ...
        // 12-cashflow bond: invest 10000, receive 1000 per month for 12 months
        {
          ...
          const gas = parseInt((await rates.internalRateOfReturnMG(cashflows, times, tokens(0.05))).gasUsed);
          assert.equal(gas, 46617, `gas changed: ${gas} ≠ 46617 (12-cashflow) — deterministic, update threshold if intentional`);
        }
      });
    });
  });
