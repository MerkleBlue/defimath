...
contract Black76Harness {
    function callIV(uint128 future, uint128 strike, uint32 t, uint64 rate, uint128 price)
        external pure returns (uint256)
    {
        return Black76.impliedVolatility(future, strike, t, rate, price, true);
    }
    function putIV(uint128 future, uint128 strike, uint32 t, uint64 rate, uint128 price)
        external pure returns (uint256)
    {
        return Black76.impliedVolatility(future, strike, t, rate, price, false);
    }
}
...
contract Black76PropertyTest is Test {
    ...
    /// Floor was bumped 1e6 → 1e7 after a fuzz seed at future=$0.001 / t=3s / vol=169%
    ...
    uint128 private constant FUT_TYPICAL_LO = uint128(0.001e18);   //  $0.001
    uint128 private constant FUT_TYPICAL_HI = uint128(1_000_000e18); // $1M
    ...
    /// contract's accepted domain. Strike is bounded by `future/5 ≤ K ≤ future*5` per the
    /// library, which we keep tighter at `future/4 ≤ K ≤ future*4` for stability.
    function _boundInputs(uint128 future, uint128 strike, uint32 timeToExp, uint64 vol, uint64 rate)
        private pure returns (uint128, uint128, uint32, uint64, uint64)
    {
        future      = uint128(bound(future,      FUT_TYPICAL_LO, FUT_TYPICAL_HI));
        strike    = uint128(bound(strike,    future / 4,        uint256(future) * 4));
        ...
        return (future, strike, timeToExp, vol, rate);
    }
    ...
    /// The IV solver's no-arb band depends on a multi-variable interaction (future/strike
    ...
    function _boundIVInputs(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate)
        private pure returns (uint128, uint128, uint32, uint64, uint64)
    {
        future      = uint128(bound(future,      uint128(100e18),       uint128(10_000e18)));  // $100 – $10K
        strike    = uint128(bound(strike,    uint256(future) * 8 / 10, uint256(future) * 12 / 10)); // 0.8× to 1.2× future
        ...
        return (future, strike, t, vol, rate);
    }
    ...
    function test_RT_callIVRecoversVol(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public view {
        (future, strike, t, vol, rate) = _boundIVInputs(future, strike, t, vol, rate);
        uint256 price = Black76.call(future, strike, t, vol, rate);
        try harness.callIV(future, strike, t, rate, uint128(price)) returns (uint256 ivRecovered) {
    ...
    function test_RT_putIVRecoversVol(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public view {
        (future, strike, t, vol, rate) = _boundIVInputs(future, strike, t, vol, rate);
        uint256 price = Black76.put(future, strike, t, vol, rate);
        try harness.putIV(future, strike, t, rate, uint128(price)) returns (uint256 ivRecovered) {
    ...
    /// Call price is monotone non-decreasing in future (positive delta).
    /// Constrain strike to [0.5×futureHi, 2×futureLo] so the option carries meaningful value
    /// at both future points — dust-OTM options have prices below FP precision where the
    /// rounding noise can exceed the proportional slack.
    function test_MONO_callIncreasingInFuture(uint128 futureLo, uint128 futureHi, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        futureLo = uint128(bound(futureLo, FUT_TYPICAL_LO, FUT_TYPICAL_HI / 2));
        futureHi = uint128(bound(futureHi, uint256(futureLo) + 1, uint256(futureLo) * 2));
        strike = uint128(bound(strike, futureHi / 2, uint256(futureLo) * 2));   // both options near ATM
        t = uint32(bound(t, TIME_TYPICAL_LO, TIME_TYPICAL_HI));
        vol = uint64(bound(vol, VOL_TYPICAL_LO, uint64(2e18)));   // ≤ 200% vol
        rate = uint64(bound(rate, RATE_TYPICAL_LO, RATE_TYPICAL_HI));
        uint256 cLo = Black76.call(futureLo, strike, t, vol, rate);
        uint256 cHi = Black76.call(futureHi, strike, t, vol, rate);
        assertLe(cLo, cHi + _slack(cHi), "call not monotone increasing in future");
    }

    /// Put price is monotone non-increasing in future (negative delta).
    function test_MONO_putDecreasingInFuture(uint128 futureLo, uint128 futureHi, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        futureLo = uint128(bound(futureLo, FUT_TYPICAL_LO, FUT_TYPICAL_HI / 2));
        futureHi = uint128(bound(futureHi, uint256(futureLo) + 1, uint256(futureLo) * 2));
        strike = uint128(bound(strike, futureHi / 2, uint256(futureLo) * 2));
        t = uint32(bound(t, TIME_TYPICAL_LO, TIME_TYPICAL_HI));
        vol = uint64(bound(vol, VOL_TYPICAL_LO, uint64(2e18)));
        rate = uint64(bound(rate, RATE_TYPICAL_LO, RATE_TYPICAL_HI));
        uint256 pLo = Black76.put(futureLo, strike, t, vol, rate);
        uint256 pHi = Black76.put(futureHi, strike, t, vol, rate);
        assertGe(pLo + _slack(pLo), pHi, "put not monotone decreasing in future");
    }
    ...
    function test_MONO_callIncreasingInVol(uint128 future, uint128 strike, uint32 t, uint64 volLo, uint64 volHi, uint64 rate) public pure {
        future = uint128(bound(future, FUT_TYPICAL_LO, FUT_TYPICAL_HI));
        strike = uint128(bound(strike, future / 2, uint256(future) * 2));  // near ATM for clean signal
        ...
        rate = uint64(bound(rate, RATE_TYPICAL_LO, RATE_TYPICAL_HI));
        uint256 cLo = Black76.call(future, strike, t, volLo, rate);
        uint256 cHi = Black76.call(future, strike, t, volHi, rate);
    ...
    function test_MONO_putIncreasingInVol(uint128 future, uint128 strike, uint32 t, uint64 volLo, uint64 volHi, uint64 rate) public pure {
        future = uint128(bound(future, FUT_TYPICAL_LO, FUT_TYPICAL_HI));
        strike = uint128(bound(strike, future / 2, uint256(future) * 2));
        t = uint32(bound(t, 7 days, TIME_TYPICAL_HI));
        volLo = uint64(bound(volLo, VOL_TYPICAL_LO, uint64(VOL_TYPICAL_HI - 0.01e18)));
        volHi = uint64(bound(volHi, volLo + uint64(0.01e18), VOL_TYPICAL_HI));
        rate = uint64(bound(rate, RATE_TYPICAL_LO, RATE_TYPICAL_HI));
        uint256 pLo = Black76.put(future, strike, t, volLo, rate);
        uint256 pHi = Black76.put(future, strike, t, volHi, rate);
    ...
    function test_ID_putCallParity(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        uint256 callPx = Black76.call(future, strike, t, vol, rate);
        uint256 putPx  = Black76.put(future, strike, t, vol, rate);
        // e^(−rT)·(F − K)
        ...
        int256 rhs = (int256(uint256(future)) - int256(uint256(strike))) * int256(FP_ONE) / int256(discount);
    ...
    function test_ID_deltaParity(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        (int128 dCall, int128 dPut) = Black76.delta(future, strike, t, vol, rate);
    ...
    function test_ID_deltaSignAtExpiry(uint128 future, uint128 strike, uint64 vol, uint64 rate) public pure {
        future = uint128(bound(future, FUT_TYPICAL_LO, FUT_TYPICAL_HI));
        strike = uint128(bound(strike, future / 4, uint256(future) * 4));
        vol = uint64(bound(vol, VOL_TYPICAL_LO, VOL_TYPICAL_HI));
        rate = uint64(bound(rate, RATE_TYPICAL_LO, RATE_TYPICAL_HI));
        (int128 dCall, int128 dPut) = Black76.delta(future, strike, 0, vol, rate);
    ...
    function test_ID_thetaParity(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        (int128 thCall, int128 thPut) = Black76.theta(future, strike, t, vol, rate);
        ...
        int256 futMinusK = (int256(uint256(future)) - int256(uint256(strike))) * int256(FP_ONE) / int256(discount);
        int256 expected = int256(uint256(rate)) * futMinusK / int256(FP_ONE) / 365;
    ...
    /// 0 ≤ call price ≤ e^(−rT)·F — fundamental no-arbitrage upper bound (discounted-future ceiling).
    function test_BNDS_callPriceInDiscountedFutureRange(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        uint256 price = Black76.call(future, strike, t, vol, rate);
        uint256 timeYear = uint256(t) * FP_ONE / 31536000;
        uint256 discount = Math.expPositive(uint256(rate) * timeYear / FP_ONE);
        uint256 discountedF = uint256(future) * FP_ONE / discount;
        assertLe(price, discountedF + 1, "call > discounted future (arb violation)");
    }

    /// 0 ≤ put price ≤ K·e^(−rT) — discounted strike ceiling (put payoff capped at strike).
    function test_BNDS_putPriceInDiscountedStrikeRange(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        uint256 price = Black76.put(future, strike, t, vol, rate);
        uint256 timeYear = uint256(t) * FP_ONE / 31536000;
        uint256 discount = Math.expPositive(uint256(rate) * timeYear / FP_ONE);
    ...
    /// Delta call ∈ [0, e^(−rT)] (a call's price-sensitivity to future is bounded, discounted).
    function test_BNDS_deltaCallInZeroDiscount(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        (int128 dCall, ) = Black76.delta(future, strike, t, vol, rate);
        uint256 timeYear = uint256(t) * FP_ONE / 31536000;
        uint256 disc = FP_ONE * FP_ONE / Math.expPositive(uint256(rate) * timeYear / FP_ONE);  // e^(−rT)
    ...
    /// Delta put ∈ [−e^(−rT), 0].
    function test_BNDS_deltaPutInMinusDiscountZero(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        (, int128 dPut) = Black76.delta(future, strike, t, vol, rate);
        uint256 timeYear = uint256(t) * FP_ONE / 31536000;
        uint256 disc = FP_ONE * FP_ONE / Math.expPositive(uint256(rate) * timeYear / FP_ONE);  // e^(−rT)
    ...
    /// Gamma ≥ 0 (convexity of the option price wrt future).
    function test_BNDS_gammaNonNegative(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        uint256 g = Black76.gamma(future, strike, t, vol, rate);
    ...
    function test_BNDS_vegaNonNegative(uint128 future, uint128 strike, uint32 t, uint64 vol, uint64 rate) public pure {
        (future, strike, t, vol, rate) = _boundInputs(future, strike, t, vol, rate);
        uint256 v = Black76.vega(future, strike, t, vol, rate);
